...
def on_test_start(environment, runner, **kwargs):
    ...
    running_param['producer'] = KafkaProducer(value_serializer=lambda v:json.dumps(v).encode('utf-8'),bootstrap_servers=settings.tiger_kafka, api_version=(2,5,0))
...
class EchoTaskSet(TaskSet):
    def on_start(self):
        ...
        self.producer = running_param['producer']
    ...
    def send_fake_order(self):
        ...
        self.content[0]["vendor"] = self.db_vendor
        try:
            self.producer.send(self.topic, self.content)
            print(str(self.content[0]))
            events.request.fire(request_type=f'KSS',
                                name=f'kafka_producer_send_success',
                                response_time=0,
                                response_length=0,
                                exception=None,
                                context=f"kafka send content")
        except Exception as ex:
            # print(str(ex))
            self.logger.error(f"Exception : {ex}")
            events.request.fire(
                request_type=f'KSR',
                name=f'kafka_producer_send_fail',
                response_time=0,
                response_length=0,
                exception=f'{ex}'
            )
...
class StagesShape(LoadTestShape):
    stages = [{"duration": 3700,  "users": 1000,  "spawn_rate": 10 }]
    print(stages)
